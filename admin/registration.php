...
if (isset($_POST['submit'])) {
    ...
    if (strlen($_POST['adminPassword']) < 8) {
        ...
        if ($password == $cpassword) {
            $sql = "SELECT * FROM users WHERE adminEmail='$email'";
            $result = mysqli_query($conn, $sql);
            if (!$result->num_rows > 0) {
                $sql = "INSERT INTO users (adminName, adminUsername, adminEmail, adminContact, adminPassword)
                        VALUES ('$name','$adminUsername', '$email', '$contact', '$password')";
                $result = mysqli_query($conn, $sql);
                if ($result) {
                    echo '<script src="https://cdn.bootcss.com/jquery/3.3.1/jquery.js"></script>
                    <script src="https://unpkg.com/sweetalert/dist/sweetalert.min.js"></script>
...
@media screen and (min-width: 769px) {
    ...
    .container{
        ...
        margin-top: 10%; /* Adjusted margin for better centering on smaller screens */
...
<div class="text1" >
    ...
    <button onclick="window.location.href='../admin/admin_management.php';" class="btn-primary">
     Admin Management
    </button>
	<div class="container" style="margin-top: 1%;">
    <form action="" method="POST" class="login-email">
            <center><p class="login-text" style="font-size: 1.5rem; font-weight: 800;">Registration Form</p></center>
			<br><div class="input-group">
				<input type="text" placeholder="Name" name="adminName" value="<?php echo $name; ?>" required>
			</div>
            <div class="input-group">
				<input type="text" placeholder="Username" name="adminUsername" value="<?php echo $adminUsername; ?>" required>
			</div>
			<div class="input-group">
				<input type="email" placeholder="Email" name="adminEmail" value="<?php echo $email; ?>" required>
			</div>
            <div class="input-group">
				<input type="text" placeholder="Contact Number" name="adminContact" value="<?php echo $contact; ?>" required>
			</div>
			<div class="input-group">
    <input type="password" placeholder="Password" name="adminPassword" id="adminPassword" value="<?php echo $_POST['adminPassword']; ?>" required>
</div>
<div class="input-group">
